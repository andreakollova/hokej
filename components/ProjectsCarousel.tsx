      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
           <div className="relative">
             ...
             <h2 className="text-5xl md:text-6xl font-black text-white tracking-tighter uppercase leading-none drop-shadow-xl">
           ...
           <div className="flex gap-4 self-start md:self-auto">
              <button 
                onClick={() => scroll('left')}
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white hover:bg-white hover:text-slovak-blue transition-all active:scale-95 bg-white/5 backdrop-blur-sm group"
              >
                <ChevronLeft size={20} className="group-hover:-translate-x-0.5 transition-transform" />
              </button>
              <button 
                onClick={() => scroll('right')}
                className="w-10 h-10 rounded-full bg-slovak-red text-white flex items-center justify-center hover:bg-white hover:text-slovak-red transition-all shadow-lg shadow-red-900/30 active:scale-95 group border border-transparent hover:border-slovak-red"
              >
        ...
        <div 
          ref={scrollRef}
          className="flex overflow-x-auto gap-4 md:gap-8 pb-12 -mx-4 px-4 md:mx-0 md:px-0 snap-x hide-scrollbar scroll-smooth"
        >
           {PROJECTS.map((project, index) => (
             <div 
               key={project.id}
               className="min-w-[85vw] md:min-w-[350px] snap-center group cursor-pointer relative"
             >
                ...
                <div className="bg-[#0f2a55] border border-white/5 rounded-3xl overflow-hidden hover:border-white/20 transition-all duration-300 shadow-xl hover:shadow-2xl hover:shadow-blue-900/20 h-full flex flex-col">
                   ...
                   <div className="h-[200px] relative overflow-hidden">
                      ...
                      {/* Icon Badge */}
                      <div className="absolute top-4 left-4 w-12 h-12 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center text-white border border-white/20 shadow-lg transition-colors duration-300">
                         {project.icon}
                      </div>
                   </div>
