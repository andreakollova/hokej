...
export const Hero: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[500px] md:h-[580px] text-white overflow-hidden rounded-b-[40px] md:rounded-b-[60px] shadow-2xl mx-auto max-w-[1920px] group bg-slovak-blue">
      ...
      {/* Content Layer */}
      <div className="relative z-20 container mx-auto px-6 md:px-12 h-full flex flex-col justify-end pb-16 md:pb-24">
         {slides.map((slide, index) => (
           <div 
             key={slide.id} 
             className={`${index === currentSlide ? 'block' : 'hidden'} max-w-5xl space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700`}
           >
              ...
              <h1 className="text-3xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none text-white drop-shadow-xl">
                {slide.title}
              </h1>

              <p className="text-base md:text-lg text-blue-100 font-medium leading-relaxed max-w-3xl line-clamp-2 border-l-4 border-slovak-red pl-6">
                {slide.snippet}
              </p>
      ...
      {/* Navigation Controls */}
      <div className="absolute bottom-16 md:bottom-24 right-6 md:right-12 z-30 flex items-center gap-6">
